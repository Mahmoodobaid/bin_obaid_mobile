...
class _AdminScreenState extends ConsumerState<AdminScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(adminRequestProvider.notifier).loadRequests();
    });
  }
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            ...
            tabs: const [Tab(text: 'طلبات التسجيل'), Tab(text: 'استعادة كلمة المرور')],
          ),
        ),
        body: TabBarView(
          controller: _tabController,
  ...
  Widget _buildPasswordResetList(List<PasswordResetRequest> requests) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (c, i) {
        ...
        return ListTile(
          title: Text(r.phone),
          trailing: ElevatedButton(onPressed: () {}, child: const Text('إرسال')),
        );
      },
    );
  }
