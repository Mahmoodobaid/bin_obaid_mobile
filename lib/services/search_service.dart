...
class SearchService {
  ...
  Future<List<Product>> search(String query, {int limit = _defaultLimit}) async {
    ...
    if (!_isOffline) {
      _searchRemoteAndUpdateLocal(normalized, limit: limit);
    }
  ...
  Future<List<Product>> _searchLocal(String normalizedQuery, {int limit = _defaultLimit}) async {
    try {
      ...
      if (allProducts.length > 500) {
        ...
        return searchLocal(allProducts, normalizedQuery, limit: limit);
      }
    } catch (e) {
  ...
  static List<Product> _searchLocalIsolate(_SearchParams params) {
    return searchLocal(params.products, params.query, limit: params.limit);
  }

  // ---------- دالة البحث المحلي العامة (تُستخدم من الداخل ومن product_provider) ----------
  static List<Product> searchLocal(List<Product> products, String query, {int limit = 20}) {
    if (query.trim().isEmpty) return products.take(limit).toList();
    final normalized = _normalizeText(query);
    final queryTokens = normalized.split(' ').where((t) => t.isNotEmpty).toList();
  ...
  String _buildSearchableText(Product product) {
    return _buildSearchableTextStatic(product);
  }
  ...
  double _calculateRelevanceScore(String text, List<String> queryTokens) {
    return _calculateRelevanceScoreStatic(text, queryTokens);
  }
