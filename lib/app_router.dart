...
final goRouterProvider = Provider<GoRouter>((ref) { final auth = ref.watch(authProvider); return GoRouter(initialLocation: '/login', redirect: (ctx, state) { final logged = auth.isAuthenticated; final toLogin = state.matchedLocation=='/login'; final toReg = state.matchedLocation=='/register'; if(toReg) return null; if(!logged && !toLogin) return '/login'; if(logged && toLogin) return '/home'; if(state.matchedLocation.startsWith('/admin') && !auth.isAdmin) return '/home'; return null; }, routes: [
  GoRoute(path:'/login', builder:(_,__)=>const LoginScreen()), GoRoute(path:'/register', builder:(_,__)=>const RegisterScreen()),
  GoRoute(path:'/home', builder:(_,__)=>const HomeScreen()), GoRoute(path:'/catalog', builder:(_,__)=>const ProductListScreen()),
  GoRoute(path:'/product/:sku', builder:(_,s)=>ProductDetailScreen(sku: s.pathParameters['sku']!)), GoRoute(path:'/cart', builder:(_,__)=>const CartScreen()),
  GoRoute(path:'/invoice', builder:(_,__)=>const InvoiceScreen()), GoRoute(path:'/profile', builder:(_,__)=>const ProfileScreen()),
  GoRoute(path:'/settings', builder:(_,__)=>const SettingsScreen()), GoRoute(path:'/admin', builder:(_,__)=>const AdminScreen(), routes:[GoRoute(path:'database', builder:(_,__)=>const DatabaseManagerScreen())]),
  GoRoute(path:'/import', builder:(_,__)=>const ImportProductsScreen()), GoRoute(path:'/delivery', builder:(_,__)=>const DeliveryScreen()),
]); });
